...
def compilar_informe_global(tech, force_consenso=False):
    ...
    MODEL_EQUATIONS = {
        "Bass_Clasico": ("Modelo de Bass Clásico", "", "  x(t) = m * (1 - exp(-(p + q) * t)) / (1 + (q / p) * exp(-(p + q) * t))"),
        "Dual_Market": ("Modelo de Dos Mercados Independientes", "Roset & Canals", "  x(t) = x1(t) + x2(t), donde x1 y x2 son modelos clásicos de Bass independientes:\n  xi(t) = mi * (1 - exp(-(pi + qi) * t)) / (1 + (qi / pi) * exp(-(pi + qi) * t))"),
        "Fourt_Woodlock": ("Modelo de Innovación Pura", "", "  N(t) = m * (1 - exp(-p * t))"),
        "Gompertz": ("Modelo Asimétrico de Gompertz", "", "  N(t) = m * exp(-exp(-k * (t - t0)))"),
        "Generalized_Bass": ("Modelo de Bass Generalizado", "", "  dN/dt = (p + (q / m) * N(t)) * (m - N(t)) * (1 + beta * t)"),
        "Horsky_Simon": ("Modelo con Publicidad", "", "  dN/dt = (p0 + alpha * ln(1 + t) + (q / m) * N(t)) * (m - N(t))"),
        "Muller_Yogev": ("Modelo del Efecto Saddle", "", "  I(t) = Ni * (1 - exp(-(pi + qi) * t)) / (1 + (qi / pi) * exp(-(pi + qi) * t))\n  dM/dt = (pm + qm * M(t) / (Ni + Nm) + qim * I(t) / (Ni + Nm)) * (Nm - M(t))"),
        "VdB_Joshi": ("Modelo de Influenciadores e Imitadores", "", "  F1(t) = (1 - exp(-(p1 + q1) * t)) / (1 + (q1 / p1) * exp(-(p1 + q1) * t))\n  dF2/dt = q2 * (w * F1(t) + (1 - w) * F2(t)) * (1 - F2(t))\n  N(t) = M1 * F1(t) + M2 * F2(t)"),
        "Logistic_Diffusion_Convergence": ("Modelo Logístico de Difusión-Convergencia", "Ryu & Kim", "  L(t) = b1 / (1 + ((b1 - b0) / b0) * exp(-k2 * (t - t0)))"),
        "Ladron_Putsis": ("Modelo de Mercado Potencial Dinámico y Endógeno", "", "  C(t) = 1.0 - theta * exp(-gamma * N(t) / S), donde M(t) = C(t) * S y la difusión es:\n  dN/dt = (alpha + beta * (N / M)) * (M - N)")
    }
...
"""
    ...
    assert set(model_labels) == set(MODEL_EQUATIONS), f"Keys desincronizadas: {set(model_labels) ^ set(MODEL_EQUATIONS)}"

    report_md += "\n### 📐 Formulación Matemática de los Modelos Evaluados\n\n"
    
    # Invariante formal: para todo modelo en ambas secciones, el label de model_labels es prefijo del header de formulación.
    for k, label in model_labels.items():
        ...
        if yr:
            ...
            header_content = label + ((" (" + autores + ")") if autores else "")
            
        report_md += "* **" + header_content + "** — " + desc + ":\n" + eq_text + "\n\n"
