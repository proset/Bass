...
(ej. `from gemini_reviewer import full_review_gemini`).
 
Para proyectos nuevos, importar directamente de llm_reviewer:
    from llm_reviewer import full_review
    issues = full_review(texto, tabla, modelos, backend="gemini")
"""
...
def review_with_gemini(narrative_text: str, tables_summary: str, **kwargs):
    """Alias que fuerza backend='gemini', para el codigo que llamaba a la version original."""
    return review_with_llm(narrative_text, tables_summary, backend="gemini", **kwargs)
 
 
def full_review_gemini(
    narrative_text: str,
    historical_table: dict,
    model_fits: List[ModelFit],
    use_llm: bool = True,
) -> List[Issue]:
    """Firma identica a la version original: siempre usa backend='gemini'."""
    from llm_reviewer import full_review as _full_review
    return _full_review(narrative_text, historical_table, model_fits, backend="gemini", use_llm=use_llm)
