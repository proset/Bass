...
def generar_consenso_pronostico_ia(tech, df_hist, params, analisis_cualitativo, recommended_model_key=None):
    ...
    0. **PROHIBICIÓN ABSOLUTA DE NÚMEROS EN PROSA**: NO escribas NINGÚN número en el texto narrativo, de NINGUNA clase: ni valores de adopción, ni incrementos, ni porcentajes, ni valores de R², MAPE o Score (ni siquiera aproximados o redondeados), ni años entre paréntesis como citación académica. ÚNICA excepción: años calendario usados exclusivamente como referencia temporal (ej. "hasta 2030", "en la próxima década que inicia en 2026"). Los modelos se citan SOLO por nombre, jamás con año entre paréntesis. Toda cifra exacta vive en las tablas y en los bloques oficiales del informe.
    ...
    8. **Líderes de métricas SIN valores**: al comparar modelos, di qué modelo lidera cada métrica ("Van den Bulte & Joshi presenta el R² más alto", "Dual Market el MAPE más bajo") SIN escribir ningún valor numérico. Nunca escribas números de R², MAPE, Score ni porcentajes de diferencia.
    9. **Verificación de líderes de métricas**: Al mencionar qué modelo tiene el "R² más alto" o el "MAPE más bajo", verifica EXACTAMENTE contra la tabla de métricas de calibración que se muestra arriba en este prompt. El líder de MAPE es el modelo con el valor MÁS BAJO en la columna MAPE (no el que "suene" como líder). El líder de R² es el modelo con el valor MÁS ALTO en R². Si la tabla muestra que el modelo A tiene MAPE=7.76% y el modelo B tiene MAPE=7.82%, el líder de MAPE es A, no B. No asumas — verifica.
    10. **Citas sin años**: NUNCA escribas un año entre paréntesis tras un nombre de modelo o de autor (ej. "Ladrón-de-Guevara & Putsis (2011)" está PROHIBIDO). Cita los modelos SOLO por su nombre: "Ladrón-de-Guevara & Putsis". El año de publicación correcto aparece en la sección de formulaciones, generada automáticamente.
